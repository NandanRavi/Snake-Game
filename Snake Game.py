...
class Snake(tk.Canvas):
    ...
    def load_assets(self):
        try:
            ...
            self.food_images = Image.open(path_to_food)
            self.food = ImageTk.PhotoImage(self.food_images)
    ...
    def create_objects(self):
        self.create_text(
            35, 12, text=f"Score: {self.score}",
            tag="score", fill="#fff", font=("TkDefaultFont", 14)
        )

        for x_positions, y_positions in self.snake_positions:
            self.create_image(
                x_positions, y_positions,image=self.snake_body, tag="snake"
            )
    ...
    def check_food_collision(self):
        if self.snake_positions[0] == self.food_positions:
            ...
            self.itemconfigure(score,
                               text=f"Score {self.score}", tag="score"
                               )
